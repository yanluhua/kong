...
Schema.validators = {
  ...
  mutually_exclusive_subsets = function(value, subsets)
    local subset_union = {} -- union of all subsets; key is element, value is the
    for _, subset in ipairs(subsets) do -- the subset it's part of
...
function Schema:validate(input, full_check)
  ...
  if self.subschema_key then
    ...
    -- TODO if subschema key is a table, check for each value or the first match?
